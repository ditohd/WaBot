...
const random = JSON.parse(fs.readFileSync('./src/random.json'))
...
const prefix = db.get('dito.prefix')
...
async function funcpremi(premi) {
	if (premi == 1) {
		reply(random.PREMIUM[1])
	}
	if (premi == 2) {
		reply(random.PREMIUM[2])
	}
	if (premi == 3) {
		reply(random.PREMIUM[3])
	}
	if (premi == 4) {
		reply(random.PREMIUM[4])
	}
	if (premi == 5) {
		reply(random.PREMIUM[5])
	}
	if (premi > 5) { }
}
...
client.on('chat-update', async (geps) => {
	try {
		if (!geps.hasNewMessage) return;
		geps = JSON.parse(JSON.stringify(geps)).messsages[0]
...
client.on('message-new', async (mek) => {
	try {
		...
		mess = {
			...
			wait: '[🚥]Loading...',
			success: 'Berhasil!',
			afkalready: 'AFK Owner is Already activated before...',
			afkgroupalready: 'AFK Owner is Already activated in this group before...',
			wrongFormat: 'Format salah, silahkan cek menu.',
			afkdisable: 'AFK Berhasil dimatikan!',
			afkenable: 'AFK berhasil diaktifkan!',
			publikdisable: 'Public Commands Activated!',
			publikenable: 'Public Commands Stopped!',
			publiksaha: 'Owner Bot only!',
			sedangafk: 'Maaf dito sedang offline, silahkan hubungi nanti...\n\n_Junito Milk_',
			error: {
				stick: 'Stiker yaa, rawr :3',
				Iv: 'Linknya invalid / tidak tersedia.'
			},
			only: {
				group: 'Group Chat Only.',
				ownerG: 'Owner Bot dulu lah',
				ownerB: 'Lahh?',
				admin: 'Admin group only...\nHanya admin group...',
				Badmin: 'Jadiin admin group, biar bisa diaktifin fitur ini!'
			}
		}
		...
		const isUsername = new RegExp(/^[a-zA-Z\-]+$/g)
		...
		var premi = Math.floor((Math.random() * 10) + 1)
...
• Link: https://www.instagram.com/${anu.result.username}`
					...
					case 'eval':
						try {
							...
							var evaled = eval(argumen);
							if (typeof evaled !== 'string')
								evaled = require('util').inspect(evaled);
							client.sendMessage(from, `\`\`\`xl\n${clean(evaled)}\n\`\`\``, text, { quoted: mek });
						} catch (err) {
							client.sendMessage(from, `\`ERROR\` \`\`\`xl\n${clean(err)}\n\`\`\``, text, { quoted: mek });
							break
						}
						break
					case 'tomp3':
